...
def encoder(x, n_code, phase_train):
    with tf.variable_scope("encoder"):
        with tf.variable_scope("hidden_1"):
            #784次元の入力をn_endocder_hidden_1次元にする
...
if __name__ == '__main__':
    #コマンドラインで引数を受け付ける設定…コードの層に含まれるニューロン数をいじれる
    parser = argparse.ArgumentParser(description='Test various optimization strategies')#help表示
    parser.add_argument('n_code', type=int)#受け付けた値の名前
    args = parser.parse_args()#引数をオブジェクト化する
    n_code = args.n_code
            ...
            for epoch in range(training_epochs):
                ...
                for i in range(total_batch):
                    ...
                    _, new_cost, train_summary = sess.run([train_op,cost,train_summary_op],
                        feed_dict={x: mbatch_x,phase_train: True})
                ...
                if epoch % display_step == 0:
                    ...
                    saver.save(
                        sess,
                        "{}/model-checkpoint-{:04d}".format(log_dir,epoch + 1),global_step=global_step)
