...
def inference(x, keep_prob):
    #shape = (データ数) * (2d画像の次元) * (2d画像の次元) * (color) 
    x = tf.reshape(x, shape=[-1, 28, 28, 1])
    with tf.variable_scope("conv_1"):
        #5*5の広がりを持つフィルタが32
        #厚み1のデータを受け取って、32層にする
        conv_1 = conv2d(x, [5, 5, 1, 32], [32])# 28*28
        pool_1 = max_pool(conv_1)# 14*14

    with tf.variable_scope("conv_2"):
        conv_2 = conv2d(pool_1, [5, 5, 32, 64], [64])# 14*14
        pool_2 = max_pool(conv_2)# 7*7
