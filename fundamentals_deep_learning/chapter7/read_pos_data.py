...
'''
データセットをgensimで扱える形に変換する
毎回呼び出すと処理が重いので、軽量なlevelDBに変換する
'''
...
if not os.path.isdir("data/word2vecdb"):
    ...
    #使用しているデータセットでは、単語と品詞がペアになって1行ずつ入っている
    '''
    to TO
    take VB
    '''
    ...
    while counter < len(train_dataset_raw):
        ...
        #gensimのモデルを参考にして、'new' 'york'を'new_york'のように変換
        #pair[0]は単語を、pair[1]は品詞を表している
        ...
        word = re.sub("\d", "#", pair[0])#数字を#に置き換える
        word = re.sub("-", "_", word)#-を_に置き換える
    ...
    for pair in train_dataset + test_dataset:
        dataset_vocab[pair[0]] = 1#dictionary型
        #tags_to_index:品詞のリスト　に追加
    ...
    for word in dataset_vocab:
        ...
        #gensimのモデルに含まれているものは、levelDBのインスタンスにキャッシュする
        if word in model:
            db.Put(word.encode(), model[word])
        #gensimのモデルに含まれておらず2回目以降のものは、以前の値を再利用
        elif word in nonmodel_cache:
            db.Put(word.encode(), nonmodel_cache[word])
        #gensimのモデルに含まれておらず初回のものは、適当な値を振る
        else:
            print(word)
